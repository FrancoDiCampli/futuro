<form action="{{route('update.student.profile',$student)}}" method="POST" enctype="multipart/form-data">
    @method('PUT')
    @csrf
...
<div class="flex flex-wrap row">
    ...
    <div class="w-6/12 px-10 py-5 ">
        @if(isset(user()->profile->avatar->path) )
            <img class="h-20 mx-auto rounded-full shadow-sm" src="{{asset('storage/'.user()->profile->avatar->path) }}" alt="user">
        @else
