...
<div class="card bg-white p-5 flex justify-start items-start my-5 border">
    ...
    <div class="text-left mx-5 text-sm">
        <h1 class="font-semibold">{{$student->first_name}} {{$student->last_name}}</h1>
        <div class="flex items-center  text-xs">
            ...
            <p class="mx-2">{{$student->city->name}}</p>
        </div>
        <div class="flex my-5">
            <p>{{$student->subcategory->name}}</p>
        ...
        <a class="text-xl text-main-blue my-2" href="{{route('get.postulation',['student'=>$student->id,'vacancy'=>$vacancy->id])}}">{{$student->title}}</a>

        <p class="">{{$student->speech}}</p>
